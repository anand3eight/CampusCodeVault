...
int *sortSRTF(Process *prs, int n)
{
      int temp;
...
void SRTF(Process *prs, int *ord, int cst, int n)
{
      ...
      while (cmp != n)
      {
            ...
            for (int i = 0; i < n; i++)
            {
                  if (prs[ord[i]].at <= t)
                  {
                        if (prs[ord[i]].rbt > 0 && min > prs[ord[i]].rbt)
                        {
                              min = prs[ord[i]].rbt;
                              in = i;
                        }
...
int main()
{
      ...
      int *order = sortSRTF(prs, n);
      SRTF(prs, order, cst, n);
