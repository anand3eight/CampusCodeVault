...
void findWaitingTime(int *prs, int n, int *bt, int *at, int *wt){
	...
	while(c != n){
		...
		for(int i = 0; i < n; i++){
			if(at[i] <= t && (rembt[i] > 0 && min > rembt[i])){
				min = rembt[i];
				shortest = i;
				done = false;
				}
...
void findAvg(int *prs, int n, int *bt, int *at){
	int wt[n], tat[n], twt = 0, ttat = 0;
	findWaitingTime(prs, n, bt, at, wt);
